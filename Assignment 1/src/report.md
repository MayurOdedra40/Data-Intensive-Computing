...
**Figure 1 — pipeline and `<key, value>` design.** See `pipeline.dot`
(rendered to PDF/PNG with `dot -Tpdf pipeline.dot -o pipeline.pdf`).
...
The stopword set is loaded once per worker (`@lru_cache`) to avoid
...
shuffle for corpus-level counts. This is a classic **map-side broadcast
join**: small side broadcast, large side partitioned.
...
Job 4 needs — no further reshuffling is required.
...
### 3.6 Job 4 — Top-K per category (`job4_topk.py`)

Purpose: for each category, retain only the 75 highest-scoring terms.

| Stage        | Key                                              | Value                       |
|--------------|--------------------------------------------------|-----------------------------|
| mapper in    | raw text line `"<jcat>\t<jval>"`                 | —                           |
...
The output of Job 4 is already tiny, so the last formatting step runs as
...
- **Broadcast join for globals.** Keeps Job 2 a single MapReduce stage
  ...
  with term counts.
- **No combiner in Job 2 / Job 4.** Mapper output there is already
...
in Job 2, and keying Job 2's output by category so Job 4 needs no
...
The structure should comfortably meet the under-20-minute reference
target on the full 56 GB input, since each job either scans the data
once (Job 1), processes a single shuffle of per-term counts (Job 2), or
works on a data set already reduced to thousands of rows (Job 4 and
finalizer).
